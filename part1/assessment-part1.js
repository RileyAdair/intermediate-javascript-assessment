...
String.prototype.grammarPolice = function(str) {
  var arr = this.toLowerCase().split(' ').map(val => val[0].toUpperCase() + val.slice(1));
  return arr.join(' ');
}
...
function valueType(a, b){
  if(typeof a === typeof b && a === b){return 'Exactly the same'}
  else if(Number(a) === b || a === Number(b)){return 'Same value, different types'}
